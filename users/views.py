...
class RegisterView(APIView):
    def post(self, request):
        ...
        send_mail(
            'Registration code',
            message=code,
            from_email='aaskerbekova161@gmail.com',
...
class LoginAPIView(APIView):
    def post(self, request):
        ...
        return Response(status=status.HTTP_401_UNAUTHORIZED, data={'message': 'неверные данные'})
...
class SMSCodeConfirm(APIView):
    def post(self, request):
        ...
        if not serializer.is_valid():
            return Response(serializer.errors, status=400)
        code = serializer.validated_data.get('sms_code')
        try:
        ...
        sms_code.user.save()
        sms_code.delete()
        return Response(status=status.HTTP_200_OK, data={'message': 'смс подтвержден'})
